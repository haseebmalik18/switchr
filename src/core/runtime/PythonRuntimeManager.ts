...
export class PythonRuntimeManager extends RuntimeManager {
  ...
  private async setupVirtualEnvironment(_version: string): Promise<void> {
    const venvPath = path.join(this.projectPath, '.venv');
  ...
  private async cleanupProjectEnvironment(_version: string): Promise<void> {
    const venvPath = path.join(this.projectPath, '.venv');

    if (await fs.pathExists(venvPath)) {
      await fs.remove(venvPath);
      this.log('debug', `Cleaned up virtual environment for Python ${_version}`);
    }
  }
}
